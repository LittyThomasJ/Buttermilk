...
//$id=$_GET['id'];
//echo $id;
//$sel="SELECT tb FROM tb_product where login_id=$lid";
...
<head>
    ...
    <!--<script type="text/javascript" language="javascript" src="javascripts/cart.js"></script>-->
...
<li class="sp-menu-item"><a  href="logout.php"  >Logout</a></li>
		...
		</div></div></div></div></header><section id="sp-page-title"><div class="row"><div id="sp-title" class="col-sm-12 col-md-12"><div class="sp-column "><div class="sp-page-title"><div class="container">
<ol class="breadcrumb">
...
<form name="cart" method="post" id="cart" action="cartbuy.php">
	...
	<table class="table table-bordered" id="qu" name="qu">
		...
		while($row=mysqli_fetch_array($select))
							   	...
							   	?>
		<tr class="cart-row">
			<!--<td><input type="checkbox" class="case" name="case[]" id="case[]" onclick="myfunc(this);" value="<?php echo $row['product_id']; ?>"> </td>-->
			
			<td><img src="web_admin/upload/<?php echo $row['picture']; ?>" width="150px" height="100px"></td>
<!--<td><input type="hidden" name="proid" value="<?php echo $row['product_id'];?>" id="proid"></td>-->
<td style="display: none"><input type="text" name="productid[]" value="<?php echo $row['product_id']; ?>" id="product_id[]"></td>
<td style="display: none"><input type="text" name="cartid[]" value="<?php echo $row['cart_id']; ?>" id="cart_id[]"></td>
			<td><?php echo $row['product'];?></td>
			
			

			<td><input type="number" name="quantity[]" id="quantity" class="cart-variant--quantity_input" placeholder="no. of items" init="0" max="<?php echo $row['items']; ?>"  min="1" max="<?php echo $row['items'];?>" required data-unit-price="<?php echo $row['price'];?>">
			<span id="quantity_error_message" style="color:red" ></span></td>
			<td><?php echo $row['price'];?></td>
			<td><select name="pack[]" class="form-control" id="pack" required> 
					 													...
					 													<span id="pack_error_message" style="color:red" ></span></td>
					 														<td><select name="deli[]" class="deli" id="deli" required> 
																			  ...
																			  $queee=$obj->execute($que);
																			      ...
																			      ?>
					 														</select><span id="deli_error_message" style="color:red" ></span></td>
					 														<td ><center><a href="delcartaction.php?id= <?php echo $row['product_id']; ?>" onclick="return confirm_alert(this);">Remove</a><!--<button class="btn" href="block.php?id= <?php echo $row['product_id']; ?>"  style=" 
  background-color: DodgerBlue;
  border: none;
  color: white;
  padding: 12px 16px;
  font-size: 16px;
  cursor: pointer;" onMouseOver="this.style.color='red'" onMouseOut="this.style.color='white'"
><i class="fa fa-trash"></i> Trash</button>--></center></td>
	...
	?>
	<tr >
		<td colspan="8"><b><span id="total_error_message" style="color:#1D75D3"></span></b></td>
	</tr>
	<script >
    ...
    $("#total_error_message").html("Subtotal : " + st);
...
<tfoot >
	...
	<td colspan="8"><center><input type="submit" name="submit" id="submit" class="addtocart-button" value="Buy Now" title="Check out"  onclick="window.location='purchasedetails.php" style=" 
  background-color: DodgerBlue;
  border: none;
  color: white;
  padding: 12px 16px;
  font-size: 16px;
  cursor: pointer;"/></center></td></tr>
	...
	<!--<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="glyphicon glyphicon-trash">Remove</th>
				</tr>
				</thead>
			<tfoot>
				<tr>
			...
			<tbody>
					...
					<td class="cart-product-sub-total"><span class="cart-sub-total-price">.00</span></td>
<td class="cart-product-sub-total"><span class="cart-sub-total-price">.00</span></td>
